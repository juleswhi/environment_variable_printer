...
-opaque subject(FMQ) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FMQ}.

-type do_not_leak() :: any().

-type selector(FMR) :: any() | {gleam_phantom, FMR}.
...
-type call_error(FMS) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FMS}.
...
-spec select(selector(FNJ), integer()) -> {ok, FNJ} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-spec select_forever(selector(FNN)) -> FNN.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-spec map_selector(selector(FNP), fun((FNP) -> FNR)) -> selector(FNR).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-spec merge_selector(selector(FNT), selector(FNT)) -> selector(FNT).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(FNX), fun((exit_message()) -> FNX)) -> selector(FNX).
...
-spec selecting(selector(FOA), subject(FOC), fun((FOC) -> FOA)) -> selector(FOA).
...
-spec 'receive'(subject(FND), integer()) -> {ok, FND} | {error, nil}.
...
-spec selecting_record2(
    selector(FOF),
    any(),
    fun((gleam@dynamic:dynamic_()) -> FOF)
) -> selector(FOF).
...
-spec selecting_record3(
    selector(FOJ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FOJ)
) -> selector(FOJ).
...
-spec selecting_record4(
    selector(FON),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FON)
) -> selector(FON).
...
-spec selecting_record5(
    selector(FOR),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FOR)
) -> selector(FOR).
...
-spec selecting_record6(
    selector(FOV),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FOV)
) -> selector(FOV).
...
-spec selecting_record7(
    selector(FOZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FOZ)
) -> selector(FOZ).
...
-spec selecting_record8(
    selector(FPD),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FPD)
) -> selector(FPD).
...
-spec selecting_anything(selector(FPH), fun((gleam@dynamic:dynamic_()) -> FPH)) -> selector(FPH).
...
-spec try_call(subject(FPS), fun((subject(FPU)) -> FPS), integer()) -> {ok, FPU} |
    {error, call_error(FPU)}.
...
-spec call(subject(FPZ), fun((subject(FQB)) -> FPZ), integer()) -> FQB.
...
-spec send_after(subject(FQE), integer(), FQE) -> timer().
